...
void setup() {
    ...
    if (eeprom.isModuleEnabled(EE_MODULE_NFC)) {
        nfcModule.init();      // CH9326 auto-disabled via mutual exclusion
    }
    if (eeprom.isModuleEnabled(EE_MODULE_IBUTTON)) {
        ibutton.init();
    }
    if (eeprom.isModuleEnabled(EE_MODULE_CC1101)) {
        subGHz.init();         // RFID auto-disabled via mutual exclusion
    }
    if (eeprom.isModuleEnabled(EE_MODULE_RFID)) {
        rfidModule.init();     // CC1101 auto-disabled via mutual exclusion
    }
    if (eeprom.isModuleEnabled(EE_MODULE_BADUSB)) {
        badUSB.init();         // NFC auto-disabled via mutual exclusion
    }
