...
void setup() {
    ...
    if (eeprom.isModuleEnabled(EE_MODULE_NFC)) {
        nfcModule.init();      // OLED: all modules can run together
    }
    if (eeprom.isModuleEnabled(EE_MODULE_IBUTTON)) {
        ibutton.init();
    }
    if (eeprom.isModuleEnabled(EE_MODULE_CC1101)) {
        subGHz.init();         // OLED: all modules can run together
    }
    if (eeprom.isModuleEnabled(EE_MODULE_RFID)) {
        rfidModule.init();     // OLED: all modules can run together
    }
    if (eeprom.isModuleEnabled(EE_MODULE_BADUSB)) {
        badUSB.init();         // OLED: all modules can run together
    }
